                              ...
                              <tfoot style="margin: auto">
                                <div style="position: relative; width: 100%">
                                  ...
                                  >
                                    <img
                                      :src="
                                        'data:image/png;base64,' + barcodeImage
                                      "
                                      alt="barcode"
...
const checkoutData = computed(() => {
  ...
  const vehicleNo = item.vehicle?.number,
    ...
    checkInTime = formatDate(item.vehicle?.in_time, "DD-MM-YYYY hh:mm A");
  return [
    {
      key: "Checkin Time",
      value: checkInTime,
...
const exportImage = async () => {
  try {
    if (captureRef.value) {
      ...
      link.href = dataUrl;
      link.download = parking.value?.barcode ? parking.value?.barcode + "checkin.png" : "exported-content.png";
      link.click();
    }
  } catch (error) {
